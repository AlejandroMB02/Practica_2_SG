...
class MyScene extends THREE.Scene {
  ...
  onPointerDown(event) {
    //  Si ya había una pieza seleccionada, cortamos la propagación
    if (this.piezaSeleccionada) {
      event.stopPropagation();
      event.preventDefault();
    }

    //  Calculamos coords normalizadas del ratón
    ...
    if (!this.piezaSeleccionada) {
      ...
      if (hits.length > 0) {
        // ¡Aquí paramos todo rastro de TrackballControls!
        event.stopPropagation();
        event.preventDefault();
    ...
    //  Si ya había una pieza seleccionada, intentamos soltarla en una casilla
  } else {
  ...
  if (sqHits.length > 0) {
    ...
    if (casilla.userData.resaltada) {
      // movemos la pieza
      this.piezaSeleccionada.position.x = casilla.position.x;
      this.piezaSeleccionada.position.z = casilla.position.z;
      this.piezaSeleccionada.userData = {
        fila: casilla.userData.fila,
        columna: casilla.userData.columna
      };
      this.model.limpiarResaltado();
      this.piezaSeleccionada = null;
      // volvemos a activar cámara
      this.cameraControl.noRotate = false;
      this.cameraControl.noPan = false;
    }
  } else {
    // clic fuera de casilla: cancelar
    this.model.limpiarResaltado();
    this.piezaSeleccionada = null;
    this.cameraControl.noRotate = false;
    this.cameraControl.noPan = false;
