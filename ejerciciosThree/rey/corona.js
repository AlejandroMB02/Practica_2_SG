...
function crearLapiz() {
     var material_amarillo = new THREE.MeshStandardMaterial({color: 0xFBFF00});
            ...
            //Geometrias
            var cuerpo_lapiz = new THREE.CylinderGeometry(0.007, 0.007, 0.02, 6);
            var cuerpo_lapiz_2 = new THREE.CylinderGeometry(0.007, 0.007, 0.01, 6);
            var madera = new THREE.CylinderGeometry(0.002, 0.01, 0.02, 30);
            var punta = new THREE.CylinderGeometry(0, 0.002, 0.004, 30);
    
            //Transformaciones
            madera.translate(0,0.01,0);
            cuerpo_lapiz.translate(0,0.01,0);
            punta.translate(0,0.022,0);
...
class Corona extends THREE.Object3D{
    constructor(){
        ...
        // Crear un grupo para instanciar múltiples lápices en círculo
        const group = new THREE.Group();
        const numPencils = 10;
        const radius = 0.016;

        for (let i = 0; i < numPencils; i++) {
            const angle = (i / numPencils) * Math.PI * 2;
            // Clonar el CSG.Brush o Mesh según lo devuelto por crearLapizBrush
            const clone = lapiz.clone();
            clone.name = 'lapiz${i}';
            clone.position.set(
